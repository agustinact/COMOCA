...
def reporte_romaneo(lista_items):
    n_reporte =  datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    wb = load_workbook('Reportes/Romaneos/Reporte_Romaneos.xlsx')
    excel_file = f'Reportes/Romaneos/RROM{n_reporte}.xlsx'
    hoja = wb.worksheets[0]
    # numero factura
    # hoja['D1'] = lista_items[0]
    ...
    hoja[f'G{indice_fila}'] = str(total_peso)
